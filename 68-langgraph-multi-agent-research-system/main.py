...
def report_writer(state: ResearchState) -> dict:
    ...
    response = creative_llm.invoke(
        [
            SystemMessage(
                content=(
                    "Your are a report writer. Produce a well-structured research report "
                    "with these sections:\n"
...
def quality_checker(state: ResearchState) -> dict:
    ...
    review = review_llm.invoke(
        [
            SystemMessage(
                content=(
                    "You are a quality reviewer. Evaluate the report on:\n"
    ...
    return {
        ...
        "messages": [
            AIMessage(
                content=(
                    f"[QUALITY CHECKER]: Score {review.score:.1f} — "
...
def demo_research_with_streaming():
    """Demo the research system with step-by-step streaming output."""
