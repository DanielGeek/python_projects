...
def build_content_team():
    ...
    def content_writer(state: TeamState) -> dict:
        ...
        response = llm.invoke(
            [
                SystemMessage(
                    content="You are a skilled content writer. Using any research or context "
    ...
    def content_editor(state: TeamState) -> dict:
        ...
        return {
            "messages": [
                AIMessage(
                    content=f"[EDITOR]: {response.content}",
                    name="content_editor",
                )
            ],
            "final_answer": response.content,
...
def build_analysis_team():
    """Builds the analysis department subgraph."""
    ...
    def data_analyst(state: TeamState) -> dict:
        ...
        return {
            "messages": [
                AIMessage(
                    content=f"[DATA_ANALYST]: {response.content}",
                    name="data_analyst",
                )
            ]
        }
    ...
    def strategy_advisor(state: TeamState) -> dict:
        ...
        return {
            "messages": [
                AIMessage(
                    content=f"[STRATEGY_ADVISOR]: {response.content}",
...
def create_hierarchical_system():
            ...
            [
                SystemMessage(
                    content=(
                        ...
                        "- research: Fact-finding, investigation, technical depp-dives\n"
    ...
    # Build parent graph - departments are compiled subggraphs as nodes
...
def demo_hierarchical_routing():
    ...
    queries = [
        ...
        "Should my startup invest in building AI features this years?",
    ]
