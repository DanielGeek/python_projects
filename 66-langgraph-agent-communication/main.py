...
def create_shared_fields_pipeline():
    ...
    def data_collector(state: SharedFieldsState) -> dict:
        """Collect data and writes to the raw_data field."""
        response = llm.invoke(
            [
                SystemMessage(
                    content=(
                        ...
                        "Return only the JSON array, not markdown."
                    )
                ),
            ...
            [
                SystemMessage(
                    content=(
                        ...
                        "2) A confidence socre from 0.0 to 1.0. "
    ...
    def advisor(state: SharedFieldsState) -> dict:
        """Reads analysis + confidence, writes recommendation."""
        response = llm.invoke(
            [
                SystemMessage(
                    content=(
...
def demo_shared_state():
    ...
    result = agent.invoke(
        {
            "query": "Should a small business invest in AI amutomation in 2026?",
            "raw_data": [],
            "analysis": "",
            "recommendations": [],
...
def create_blackboard_system():
    ...
    def drafter(state: BlackboardState) -> dict:
        ...
        if state["drafts"]:
            context_parts.append(f"Previous drafts: {state['drafts'][-1]}")
        if state["critiques"]:
    ...
    def critic(state: BlackboardState) -> dict:
        ...
        result = {
            ...
            "messages": [
                AIMessage(
                    content=f"[CRITIC]: {'APPROVED' if approved else 'REVISION NEEDED'}{' - ' + decision.feedback if not approved and decision.feedback.strip() else ''}",
                    name="critic",
                )
            ],
        }
