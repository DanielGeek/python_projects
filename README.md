...
- **[86-MLOps]** - Complete 25-module MLOps curriculum covering Python foundations, NumPy/Pandas, Logging, Flask REST APIs, MLflow experiment tracking, DVC data versioning, DagsHub remote S3 storage ([demo-dagshub](https://dagshub.com/DanielGeek/demo-dagshub)), end-to-end reproducible ML pipelines ([machine-learning-pipeline](https://dagshub.com/DanielGeek/machine-learning-pipeline)), self-hosted MLflow servers on AWS EC2 & S3 (`17-mlflow-aws`), Docker containerization (`18-docker-hello-world`), Docker Compose multi-container services (`19-docker-compose`), Apache Airflow workflow orchestration with Astronomer Astro CLI (`20-airflow-astro`), production ETL pipeline extracting NASA APOD API data into PostgreSQL via Airflow DAGs (`21-ETL-pipeline`), and full CI/CD pipeline with GitHub Actions for automated testing, multi-platform Docker builds (amd64 + arm64), Docker Hub publishing (`22-github-action-docker`), and end-to-end modular Wine Quality prediction pipeline with DagsHub MLflow tracking, automated 5-stage pipeline, model registry, and interactive Flask web UI ([23-data-science-project](https://github.com/DanielGeek/data-science-project)), enterprise Network Security phishing detection pipeline with MongoDB Atlas cloud ingestion, Kolmogorov-Smirnov drift detection, multi-model GridSearchCV tuning (>99.1% F1-score), DagsHub MLflow tracking & model registry, automated AWS S3 artifact sync, FastAPI batch prediction UI, and full GitHub Actions CI/CD to AWS ECR & EC2 ([24-network-security](https://github.com/DanielGeek/network-security)), and Student Exam Performance Predictor — end-to-end regression MLOps pipeline with modular ingestion/transformation/training, multi-model selection (XGBoost, CatBoost, scikit-learn), Flask inference UI, Docker packaging, and GitHub Actions CI/CD to AWS ECR & Elastic Beanstalk ([25-ML_project](https://github.com/DanielGeek/ML_project)).
