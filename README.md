...
- **[86-MLOps]** - Complete 22-module MLOps curriculum covering Python foundations, NumPy/Pandas, Logging, Flask REST APIs, MLflow experiment tracking, DVC data versioning, DagsHub remote S3 storage ([demo-dagshub](https://dagshub.com/DanielGeek/demo-dagshub)), end-to-end reproducible ML pipelines ([machine-learning-pipeline](https://dagshub.com/DanielGeek/machine-learning-pipeline)), self-hosted MLflow servers on AWS EC2 & S3 (`17-mlflow-aws`), Docker containerization (`18-docker-hello-world`), Docker Compose multi-container services (`19-docker-compose`), Apache Airflow workflow orchestration with Astronomer Astro CLI (`20-airflow-astro`), production ETL pipeline extracting NASA APOD API data into PostgreSQL via Airflow DAGs (`21-ETL-pipeline`), and automated CI/CD pipelines with GitHub Actions (push/pull_request triggers, pytest automation, secrets management) ([app-github-action](https://github.com/DanielGeek/app-github-action)).
